...
public:
	...
	void Reset()
	{
		pq[FORWARD] = priority_queue <pair<int, int>, vector<pair<int, int>>, Compare >(); // reset it
		pq[REVERSE] = priority_queue <pair<int, int>, vector<pair<int, int>>, Compare >(); // reset it
	...
	int ShortestPath()
	{
		...
		it=set_union (proc[FORWARD].begin(), proc[FORWARD].end(), proc[REVERSE].begin(), proc[REVERSE].end(), v.begin());
		v.resize(it-v.begin());
